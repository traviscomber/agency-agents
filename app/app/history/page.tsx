    ...
    <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8 sm:py-10">
        ...
        <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
          ...
          <div className="grid grid-cols-3 gap-3 sm:min-w-[360px]">
            <div className="rounded-2xl border border-slate-200 bg-white p-3">
              <p className="text-[11px] uppercase tracking-[0.2em] text-slate-700">Total</p>
              <p className="mt-1 text-2xl font-semibold text-foreground">{MOCK_RUNS.length}</p>
            </div>
            <div className="rounded-2xl border border-slate-200 bg-white p-3">
              <p className="text-[11px] uppercase tracking-[0.2em] text-slate-700">Done</p>
      ...
      {MOCK_RUNS.length === 0 ? (
        <div className="mt-6 rounded-[28px] border border-slate-200 bg-white p-12 text-center">
          <div className="mx-auto mb-4 flex h-14 w-14 items-center justify-center rounded-2xl border border-slate-200 bg-gradient-to-br from-slate-50 to-white">
            <Bot size={26} className="text-slate-700" />
          </div>
          ...
          <aside className="space-y-4">
            <div className="rounded-[28px] border border-slate-200 bg-white p-5">
              <p className="text-xs font-semibold uppercase tracking-[0.22em] text-slate-700">Signals</p>
              <div className="mt-4 space-y-3">
                <div className="rounded-2xl border border-slate-200 bg-gradient-to-br from-slate-50 to-white p-4">
                  <p className="text-xs text-slate-700">Completion rate</p>
                  <p className="mt-1 text-2xl font-semibold text-foreground">
                    {MOCK_RUNS.length === 0 ? '0%' : `${Math.round((completedRuns / MOCK_RUNS.length) * 100)}%`}
                  </p>
                </div>
                <div className="rounded-2xl border border-slate-200 bg-gradient-to-br from-slate-50 to-white p-4">
                  <p className="text-xs text-slate-700">Failure rate</p>
                  <p className="mt-1 text-2xl font-semibold text-foreground">
            ...
            <div className="rounded-[28px] border border-slate-200 bg-white p-5">
              <p className="text-xs font-semibold uppercase tracking-[0.22em] text-slate-700">Next step</p>
