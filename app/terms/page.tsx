import Link from 'next/link'
import { PublicNavbar } from '@/components/public/PublicNavbar'

const SECTIONS = [
  {
    title: 'Using the service',
    body:
      'N3uralia Studio is provided as a product workspace and operating prototype. You may browse, test, and explore the available workflows, but you remain responsible for how you use any output generated by the product.',
  },
  {
    title: 'Accounts and access',
    body:
      'If account features are enabled, you must provide accurate information and keep your access credentials secure. We may suspend access if activity appears abusive, fraudulent, or harmful.',
  },
  {
    title: 'Content and output',
    body:
      'Outputs generated by agents are intended to support your work. You should review them before using them in production, publishing them, or relying on them for decisions.',
  },
  {
    title: 'Service changes',
    body: 'We may change, pause, or remove features at any time as the product evolves.',
  },
]

export default function TermsPage() {
  return (
    <div className="min-h-screen bg-[radial-gradient(circle_at_top_left,_rgba(15,23,42,0.08),_transparent_30%),linear-gradient(to_bottom,_#f8fafc,_#ffffff_22%,_#f8fafc_100%)]">
      <PublicNavbar />

      <main className="mx-auto max-w-4xl px-4 py-16 sm:px-6 sm:py-20">
        <section className="overflow-hidden rounded-[2rem] border border-slate-200 bg-[linear-gradient(135deg,#ffffff_0%,#ffffff_56%,#f8fafc_100%)] p-6 shadow-[0_18px_60px_-44px_rgba(15,23,42,0.45)] sm:p-8">
          <p className="mb-4 text-xs font-semibold uppercase tracking-[0.24em] text-slate-700">Terms</p>
          <h1 className="text-4xl font-semibold tracking-tight text-balance text-foreground">Terms of service</h1>
          <p className="mt-4 max-w-2xl text-sm leading-relaxed text-slate-700">
            These terms describe the basic rules for using the N3uralia Studio website, account features, and operating workspace.
          </p>
        </section>

        <div className="mt-8 space-y-4">
          {SECTIONS.map((section) => (
            <section
              key={section.title}
              className="rounded-[1.5rem] border border-slate-200 bg-white p-6 shadow-[0_12px_36px_-30px_rgba(15,23,42,0.35)]"
            >
              <h2 className="text-lg font-semibold text-foreground">{section.title}</h2>
              <p className="mt-3 max-w-3xl text-sm leading-relaxed text-slate-700">{section.body}</p>
            </section>
          ))}
        </div>

        <section className="mt-8 rounded-[1.75rem] border border-slate-200 bg-white p-6 shadow-[0_12px_36px_-30px_rgba(15,23,42,0.35)]">
          <h2 className="text-sm font-semibold text-foreground">Contact</h2>
          <p className="mt-3 text-sm leading-relaxed text-slate-700">
            If you have questions about these terms, contact{' '}
            <a href="mailto:hello@n3uralia.com" className="font-medium text-foreground hover:underline">
              hello@n3uralia.com
            </a>
            .
          </p>
        </section>
      </main>
    </div>
  )
}
