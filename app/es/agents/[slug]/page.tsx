...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  if (!agent) return {}
  return {
    title: `${agent.name} | Twin digital para Chile y Latam`,
...
export default async function SpanishAgentDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params
  return <LocalizedAgentDetailPage locale="es" slug={slug} />
}
